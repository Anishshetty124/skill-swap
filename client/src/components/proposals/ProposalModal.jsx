...
const ProposalModal = ({ isOpen, onClose, requestedSkill }) => {
  ...
  useEffect(() => {
    if (isOpen && proposalType === 'skill' && user?._id) {
      ...
      apiClient
        .get(`/skills?type=OFFER&userId=${user._id}`)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        onClose();
        navigate('/dashboard', { state: { defaultTab: 'sent_proposals' } });
      }, 700);
    } catch (err) {
  ...
  const isRequestType = requestedSkill.type === 'REQUEST';
      ...
      <div className="bg-white dark:bg-slate-800 p-8 rounded-lg shadow-xl w-full max-w-lg">
        {isSubmitting ? (
          <div className="text-center py-12">
            <Spinner text="Sending your proposal..." />
          </div>
        ) : success ? (
          <div className="text-center py-12">
            <h3 className="text-2xl font-bold text-green-500">Success!</h3>
            <p className="text-slate-500 mt-2">{success}</p>
          ...
          <>
              ...
              {isRequestType ? (
                <>
                  <p className="text-sm text-slate-600 dark:text-slate-400">
                    You are offering to teach:
                  </p>
                  <h3 className="font-semibold text-lg">
                    {requestedSkill.title}
                  </h3>
                  <p className="text-sm text-slate-600 dark:text-slate-400">
                    to {requestedSkill.user.username}
                  </p>
                </>
              ) : (
                <>
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-400">
                    from {requestedSkill.user.username}
                  </p>
                </>
              )}
            </div>
            ...
            {!isRequestType && (
              <div className="flex justify-center gap-2 mb-4 p-1 bg-slate-200 dark:bg-slate-700 rounded-lg">
                <button
                  onClick={() => setProposalType('credits')}
            ...
            <form onSubmit={handleSubmit}>
              {isRequestType ? (
                <div className="text-center p-4 border-2 border-dashed rounded-md">
                  <p className="text-slate-500">You will be paid:</p>
                  <p className="text-3xl font-bold my-2 text-amber-500">
                    {requestedSkill.creditsOffered} Credits
                  </p>
                  <p className="text-sm text-slate-500">
                    Your new balance will be:{' '}
                    {user.swapCredits + requestedSkill.creditsOffered} Credits
                  </p>
                </div>
              ) : (
                <>
                  {proposalType === 'credits' && (
                    <div className="text-center p-4 border-2 border-dashed rounded-md">
                      ...
                      <p className="text-3xl font-bold my-2 text-amber-500">
                        {requestedSkill.costInCredits} Credits
                      </p>
                      <p
                        className={`text-sm ${
                          canAfford ? 'text-slate-500' : 'text-red-500'
                        }`}
                      >
                        Your balance: {user.swapCredits} Credits
                      </p>
                    </div>
                  )}
                  {proposalType === 'skill' && (
                    <div className="mb-4">
                      ...
                      {loadingSkills ? (
                        ...
                        <select
                          ...
                          required
                          className="w-full px-3 py-2 bg-white dark:bg-slate-700 text-slate-800 dark:text-slate-200 border rounded-md"
                        >
                          <option
                            value=""
              ...
              {error && (
                <p className="text-red-500 text-sm text-center my-4">{error}</p>
              )}
              ...
              <div className="flex justify-end space-x-4 mt-6">
                <button
                  type="button"
                  onClick={onClose}
                  className="px-4 py-2 rounded-md text-slate-700 bg-slate-200 hover:bg-slate-300"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="px-4 py-2 rounded-md font-semibold text-white bg-blue-500 hover:bg-accent-700 disabled:bg-slate-400 disabled:cursor-not-allowed"
                >
