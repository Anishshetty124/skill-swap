...
        <div className="bg-slate-100 dark:bg-slate-700 p-3 rounded-md mb-6">
          ...
          {isRequestType ? (
            <>
              ...
              <p className="text-sm text-slate-600 dark:text-slate-400">to {requestedSkill.user.username}</p>
            </>
          ) : (
            <>
...
        </div>
        
        {/* Hide tabs if it's a request, as you can only accept the credits offered */}
        {!isRequestType && (
...
        <form onSubmit={handleSubmit}>
          {isRequestType ? (
            <div className="text-center p-4 border-2 border-dashed rounded-md">
              ...
              <p className="text-3xl font-bold my-2 text-amber-500">{requestedSkill.creditsOffered} Credits</p>
              <p className="text-sm text-slate-500">Your new balance will be: {user.swapCredits + requestedSkill.creditsOffered} Credits</p>
            </div>
          ) : (
            <>
...
            <button type="submit" disabled={(!canAfford && proposalType === 'credits' && !isRequestType) || !!success} className="px-4 py-2 rounded-md font-semibold text-white bg-blue-500 hover:bg-accent-700 disabled:bg-slate-400 disabled:cursor-not-allowed">
