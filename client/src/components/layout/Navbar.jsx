...
const Navbar = () => {
  ...
  // Links for Desktop View (Original Style)
  const desktopNavLinks = (
    <>
      ...
      {isAuthenticated ? (
        ...
        <>
          <Link to="/login" className="text-slate-600 dark:text-slate-300 hover:text-blue-500">Login</Link>
          <Link to="/register" className="text-slate-600 dark:text-slate-300 hover:text-blue-500 font-semibold">Register</Link>
        </>
      )}
    </>
  );
  ...
  return (
    <header className="bg-white/70 dark:bg-slate-900/70 backdrop-blur-lg shadow-sm sticky top-0 z-50 relative">
      <div className="container mx-auto px-4">
            ...
            {isAuthenticated && user && (
              <Link to={`/profile/${user.username}`}>
                {user.profilePicture ? (
                  ...
                  <div className="h-12 w-12 rounded-full bg-purple-400 text-slate-800 dark:bg-purple-700 dark:text-slate-200 flex items-center justify-center font-bold text-2xl border-2 border-slate-300 dark:border-slate-600">
            ...
            {isAuthenticated && user && (
              <Link to={`/profile/${user.username}`}>
                {user.profilePicture ? (
                  ...
                  <div className="h-11 w-11 rounded-full bg-purple-400 text-slate-800 dark:bg-purple-700 dark:text-slate-200 flex items-center justify-center font-bold text-xl border-2 border-slate-300 dark:border-slate-600">
                    {user.username.charAt(0).toUpperCase()}
                  </div>
                )}
              </Link>
        ...
        <div
          className={`absolute w-full left-0 bg-white dark:bg-slate-800 backdrop-blur-lg shadow-md md:hidden ${
