...
const ProfilePage = () => {
    ...
    // --- State for chat request loading ---
    ...
    useEffect(() => {
        const fetchProfileData = async () => {
            try {
                ...
                if (isAuthenticated && isOwner) {
                    // If there were two promises, notifications are the second result
    ...
    // --- Updated function to render the button's loading state ---
            ...
            <div className="flex flex-col md:flex-row items-center text-center md:text-left gap-6">
              <img
                ...
                src={
                  ...
                  `https://api.dicebear.com/8.x/initials/svg?seed=${profile.firstName} ${profile.lastName}`
                }
              ...
              <div>
                ...
                <div className="flex items-center justify-center md:justify-start space-x-4 mt-4 text-blue-500 font-semibold">
                  ...
                  {profile.socials?.website && (
                    <a href={profile.socials.website} target="_blank" rel="noopener noreferrer" className="hover:underline">Website</a>
                  )}
                </div>
              ...
              <div className="flex flex-wrap gap-2">
                {topCategories.length > 0 ? (
                  topCategories.map((cat) => (
                    <span
                      key={cat}
                      className="bg-blue-100 dark:bg-blue-900/50 text-blue-800 dark:text-blue-300 text-xs font-medium px-2.5 py-0.5 rounded-full"
                    >
                      {cat}
                    </span>
                  ))
              ...
              {profile.skillsToTeach?.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                  {profile.skillsToTeach.map((skill, i) => (
                    <span key={i} className="bg-green-100 dark:bg-green-900/50 text-green-800 dark:text-green-300 text-sm font-medium px-3 py-1 rounded-full">
                      {skill}
                    </span>
                  ))}
                </div>
              ...
              {profile.skillsToLearn?.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                  {profile.skillsToLearn.map((skill, i) => (
                    <span key={i} className="bg-purple-100 dark:bg-purple-900/50 text-purple-800 dark:text-purple-300 text-sm font-medium px-3 py-1 rounded-full">
                      {skill}
                    </span>
                  ))}
                </div>
            ...
            (skillsWithUser.length > 0 ? (
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {displayedSkills.map((skill) => (
                    <SkillCard key={skill._id} skill={skill} hideUser={true} />
                  ))}
                </div>
                {skillsWithUser.length > 6 && (
                  <div className="text-center mt-8">
                    ...
                    >
                      {showAllSkills
                        ? "Show Less"
                        : `Show All ${skillsWithUser.length} Skills`}
                    </button>
                  </div>
                )}
              </>
              ...
              <p className="text-slate-500 italic">
                {isOwner
                  ...
                  : "This user hasn't offered any skills yet."}
              </p>
            ))}
          {activeTab === "bookmarks" &&
            (profile.bookmarks?.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {profile.bookmarks.map((skill) => (
              ...
              <p className="text-slate-500 italic">
                {isOwner
                  ...
                  : "This user hasn't bookmarked any skills yet."}
              </p>
            ))}
          <ImageLightbox
            src={lightboxImage}
