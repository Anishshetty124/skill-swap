...
const TeamPage = () => {
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (incomingMessage) => {
      if (incomingMessage.sender._id !== user._id) {
        setTeam((prev) => ({ ...prev, chat: [...(prev.chat || []), incomingMessage] }));
      }
    };
    ...
    const handleMessageDeleted = ({ messageId }) => setTeam((prev) => ({ ...prev, chat: prev.chat.filter((msg) => msg._id !== messageId) }));
    ...
    const handleMemberConfirmed = (updatedTeam) => setTeam((prev) => ({ ...prev, completionConfirmedBy: updatedTeam.completionConfirmedBy }));
  ...
  const handleSendMessage = async (e) => {
    ...
    setTeam(prev => ({ ...prev, chat: [...prev.chat, optimisticMessage] }));
    setNewMessage("");

    try {
      const response = await apiClient.post(`/teams/${teamId}/chat`, { message: newMessage });
      const finalMessage = response.data.data;
      setTeam(prev => ({
        ...prev,
        chat: prev.chat.map(msg => msg._id === tempId ? { ...finalMessage, status: 'sent' } : msg)
      }));
    } catch (error) {
      toast.error(error.response?.data?.message || "Failed to send message.");
      setTeam(prev => ({
        ...prev,
        chat: prev.chat.map(msg => msg._id === tempId ? { ...msg, status: 'failed' } : msg)
      }));
    }
  };
  ...
  const handleDeleteTeam = async () => {
    if (confirmAction('Are you sure you want to permanently delete this team? This will refund credits to all members.')) {
      try {
  ...
  const handleClearChat = async () => {
    if (confirmAction('Are you sure you want to clear the entire chat history? This cannot be undone.')) {
      try {
  ...
  const handleLeaveTeam = async () => {
    if (confirmAction('Are you sure you want to leave this team? Your credits will be refunded.')) {
      try {
  ...
  const handleRemoveMember = async (memberId) => {
    if (confirmAction('Are you sure you want to remove this member? Their credits will be refunded.')) {
      try {
  ...
  const handleDeleteNote = async (noteId) => {
    if (confirmAction('Are you sure you want to delete this note?')) {
      try {
  ...
  const handleInitiateClosure = async () => {
    if (confirmAction('Are you sure you want to request to close this team? Members will need to confirm completion.')) {
      try {
  ...
  const handleCancelClosure = async () => {
    if (confirmAction('Are you sure you want to cancel the closure request? This will reopen the team.')) {
      try {
